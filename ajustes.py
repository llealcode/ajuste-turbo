...
def iniciar_automacao(usuario, senha, cd, departamento_responsavel, area, nv1, nv2, motivo, distribuicao):
    ...
    def esperar_carregamento_pagina():
        ...
        while True:
            try:
                icone_processando = driver.find_element(By.CSS_SELECTOR, 'div[id="WAIT_win2"]')
                sleep(0.1)
    ...
    def fazer_login_people(usuario, senha):
        ...
        driver.get('http://peoplesoft.dsp.int/psp/dpsperp_2/EMPLOYEE/ERP/c/MAINTAIN_INVENTORY.DSP_AJUSTE_EST.GBL')
    ...
    def preencher_cabecalho(cd, departamento_responsavel):
        ...
        if 'Entrada' in motivo:
            input_entrada = procurar.until(EC.element_to_be_clickable(mark=(By.CSS_SELECTOR, 'input[id="DSP_BCT_WRK_DSP_TIPO_PR_CHR$11$"]')))
            sleep(0.3)
            input_entrada.click()
            esperar_carregamento_pagina()
    ...
    def inserir_itens(df):
        ...
        for linha_item in df.index:
            ...
            input_motivo = driver.find_element(By.CSS_SELECTOR, 'input[id="DSP_BCT_DTL_TBL_REASON_CD$0"]')
            input_distribuicao = driver.find_element(By.CSS_SELECTOR, 'input[id="DSP_BCT_DTL_TBL_DISTRIB_TYPE$0"]')
            ...
            if 'Entrada' in motivo:
                driver.execute_script("arguments[0].value = 'OEE044';", input_motivo)
            else:
                driver.execute_script("arguments[0].value = 'BXE07';", input_motivo)
            ...
            input_distribuicao.send_keys(distribuicao + Keys.ENTER)
            sleep(0.1)
